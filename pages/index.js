...
const Home = () => {
  ...
  const baseUrl = `http://localhost:8000/todos/?page=${pageNum}`
  ...
  return (
    <>
      ...
      <div className={styles.prevBtn}>
        ...
        <Button className={styles.button} onClick={nextPage} disabled={data.results.length < 7}>Next</Button>
...
export const Post = ({item}) => {
  ...
  return(
    ...
    <th onClick={()=>{handleComplete(item.id)}} scope="row">{complete ? <MdCheckBox/>:<MdCheckBoxOutlineBlank/>}</th>
    <td>{item.content}</td>
